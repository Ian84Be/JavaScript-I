...
console.log(inventory.filter(car => car.id === 33)[0]);

const findId = function(idnum) {
    for (car in inventory) {
      let thisCar = inventory[car];
      if (thisCar.id == idnum) return `Car ${thisCar.id} is a ${thisCar.car_year} ${thisCar.car_make} ${thisCar.car_model}`;
    }
}
...
// UPDATED 6MAY2019
console.log(inventory.map(car => car.car_model).sort(sortString));
